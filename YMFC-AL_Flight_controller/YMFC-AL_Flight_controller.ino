...
void loop() {
  ...
  acc_total_vector = sqrt(sq(acc_x) + sq(acc_y) + sq(acc_z)); //Calculate the total accelerometer vector.
  ...
  Serial.println("#############################");
  Serial.println("Pitch acceleration: ");
  Serial.print(angle_pitch_acc);
  Serial.println("Pitch angle: ");
  Serial.print(angle_pitch);

  Serial.println("Roll acceleration: ");
  ...
  if (start == 2) {                                                         //The motors are started.
    throttle = min(throttle, 1800);                                   //We need some room to keep full control at full throttle.
    ...
    // Keep motors spinning and limit their pulse range
...
void calculate_pid() {
  ...
  pid_i_mem_roll = constrain(pid_i_mem_roll, pid_max_roll * -1, pid_max_roll);
  ...
  pid_i_mem_pitch = constrain(pid_i_mem_pitch, pid_max_pitch * -1, pid_max_pitch);
  ...
  pid_i_mem_yaw = constrain(pid_i_mem_yaw, pid_max_yaw * -1, pid_max_yaw);

  pid_output_yaw = pid_p_gain_yaw * pid_error_temp + pid_i_mem_yaw + pid_d_gain_yaw * (pid_error_temp - pid_last_yaw_d_error);
  pid_output_yaw = constrain(pid_output_yaw, pid_max_yaw * -1, pid_max_yaw);
...
int convert_receiver_channel(byte function) {
  ...
  if (actual < center) {                                                       //The actual receiver value is lower than the center value
    ...
    return 1500 + (reverse ? difference : -difference);                        //If the channel is reversed
  }
